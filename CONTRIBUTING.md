...
```sh
ldapsearch -x -b "${LDAP_BASE_DN}" -H "${LDAP_PROTOCOL_LDAP}://${LDAP_HOST}:${LDAP_PORT_LDAP}" -D "${LDAP_BIND_DN}" -w "${LDAP_BIND_PWD}" -LLL
```
...
```sh
LDAPTLS_REQCERT=never ldapsearch -x -b "${LDAP_BASE_DN}" -H "${LDAP_PROTOCOL_LDAPS}://${LDAP_HOST}:${LDAP_PORT_LDAPS}" -D "${LDAP_BIND_DN}" -w "${LDAP_BIND_PWD}" -LLL
```
...
Three connections are already set by default:
* `dummy server (plain)`
